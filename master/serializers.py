...
class ActivityHistorySerializer(serializers.ModelSerializer):
    effective_date = serializers.DateTimeField(format="%d-%m-%y %I:%M:%S %p", required=False, allow_null=True)
    created_at = serializers.DateTimeField(format="%d-%m-%y %I:%M:%S %p", read_only=True)
    status_name = serializers.SerializerMethodField()

    class Meta:
        model = activity_history
        fields = ['id', 'field_name', 'field_old_value', 'field_new_value', 'created_by', 'effective_date', 'created_at', 'status', 'status_name']
